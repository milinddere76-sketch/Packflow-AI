...
class PackFlowApp {
      ...
      <tr>
        ...
        <td>
          <div style="display:flex; gap:0.35rem;">
            ...
            <button class="btn btn-sm btn-danger" onclick="window.app.deleteCustomer('${c.id}')">
              🗑️
            </button>
          </div>
        </td>
  ...
  deleteCustomer(id) {
    if (window.appState) {
      window.appState.state.customers = window.appState.getCustomers().filter(c => c.id !== id);
      window.appState.saveState();
      this.showToast(`Customer ${id} removed!`, "info");
      this.renderCRMTable();
    }
  }
    ...
    if (macGrid) {
      macGrid.innerHTML = machines.map(m => `
        <div class="machine-card ${(m.status || 'idle').toLowerCase()}">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:0.8rem; color:var(--text-secondary);">Speed Control:</span>
            <span class="machine-speed" id="spVal_${m.id}">${m.speedMpm} MPM</span>
          </div>
          <input type="range" min="0" max="250" value="${m.speedMpm}" style="width:100%; cursor:pointer;" oninput="window.app.updateMachineSpeed('${m.id}', this.value)">
          <div style="display:flex; justify-content:space-between; align-items:center;">
  ...
  updateMachineSpeed(macId, speed) {
    if (window.appState) {
      const m = window.appState.getMachines().find(mac => mac.id === macId);
      if (m) {
        m.speedMpm = parseInt(speed) || 0;
        m.status = m.speedMpm > 0 ? "Running" : "Idle";
        m.oeePercent = Math.min(98, Math.max(50, Math.round((m.speedMpm / 200) * 100)));
        window.appState.saveState();
        const spEl = document.getElementById(`spVal_${macId}`);
        if (spEl) spEl.innerText = `${m.speedMpm} MPM`;
      }
    }
  }
  ...
  toggleMachine(macId) {
    if (window.appState) {
      ...
      this.showToast(`Machine ${macId} status toggled!`, "info");
      this.renderMRPBoard();
    }
  }
  ...
  downloadGSTR1JSON() {
        ...
        inv: [{
          ...
          itms: [{
            ...
            itm_det: {
              ...
              rt: 18,
              iamt: 0,
              camt: i.taxableValue * 0.09,
              samt: i.taxableValue * 0.09,
              csamt: 0
            }
          }]
        }]
    ...
    a.href = url;
    a.download = `GSTR1_PackFlow_082026.json`;
    a.click();
    this.showToast("GSTR-1 JSON downloaded successfully!", "success");
  }
